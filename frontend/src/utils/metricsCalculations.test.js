...
describe('calculateCellMetrics', () => {
  ...
  it('calculates totals correctly', () => {
    const cells = [
      ...
      { memory_mb: 16384, allocated_mb: 14000, used_mb: 10000, cpu_percent: 60 },
    ];
...
describe('calculateAppMetrics', () => {
  ...
  it('calculates unused percent correctly', () => {
    const apps = [
      { requested_mb: 1000, actual_mb: 750, instances: 1 },
    ];
...
describe('calculateWhatIfMetrics', () => {
  it('calculates new capacity with overcommit ratio', () => {
    const result = calculateWhatIfMetrics(32768, 1.5, 50);

    expect(result.newCapacity).toBe(49152); // 32768 * 1.5
  });

  it('calculates potential instances based on 512MB average', () => {
    const result = calculateWhatIfMetrics(32768, 1.0, 0);

    expect(result.potentialInstances).toBe(64); // 32768 / 512
    expect(result.additionalInstances).toBe(64);
  });

  it('calculates additional instances correctly', () => {
    const result = calculateWhatIfMetrics(32768, 2.0, 64);

    // New capacity: 65536, potential: 128, additional: 128 - 64 = 64
    expect(result.additionalInstances).toBe(64);
  });
});
...
describe('calculateRecommendations', () => {
  ...
  it('filters apps below threshold', () => {
    const apps = [
      { name: 'low-overhead', requested_mb: 1000, actual_mb: 900, instances: 1 }, // 10% overhead
      { name: 'high-overhead', requested_mb: 1000, actual_mb: 500, instances: 1 }, // 50% overhead
    ];
  ...
  it('sorts by total savings descending', () => {
    const apps = [
      { name: 'small-savings', requested_mb: 1000, actual_mb: 500, instances: 1 },
      { name: 'big-savings', requested_mb: 1000, actual_mb: 500, instances: 10 },
    ];
