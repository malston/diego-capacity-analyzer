...
describe('HostAnalysisCard', () => {
  ...
  describe('Host Metrics Display', () => {
    ...
    it('renders total cores', () => {
      render(<HostAnalysisCard {...defaultProps} hostCount={4} coresPerHost={32} />);
      // 4 * 32 = 128 total cores
      expect(screen.getByText('128')).toBeInTheDocument();
    });

    it('renders total memory', () => {
      render(<HostAnalysisCard {...defaultProps} hostCount={4} memoryPerHost={512} />);
      // 4 * 512 = 2048 GB = 2T
  ...
  describe('VMs Per Host Calculation', () => {
    it('displays VMs per host', () => {
      render(<HostAnalysisCard {...defaultProps} hostCount={4} totalCells={40} />);
    ...
    it('handles uneven distribution', () => {
      render(<HostAnalysisCard {...defaultProps} hostCount={3} totalCells={10} />);
  ...
  describe('HA Capacity Status', () => {
    ...
    it('shows N-1 survivable status when HA is sufficient', () => {
      ...
      render(<HostAnalysisCard {...defaultProps} hostCount={4} haAdmissionPct={25} />);
    ...
    it('shows warning when HA is insufficient', () => {
      ...
      render(<HostAnalysisCard {...defaultProps} hostCount={4} haAdmissionPct={10} />);
      expect(screen.getByText(/warning|risk/i)).toBeInTheDocument();
    });

    it('calculates hosts survivable based on HA percentage', () => {
      // 50% HA = can survive 2 host failures on 4 hosts
      render(<HostAnalysisCard {...defaultProps} hostCount={4} haAdmissionPct={50} />);
  ...
  describe('Utilization Display', () => {
    ...
    it('shows warning status for high utilization', () => {
      const { container } = render(<HostAnalysisCard {...defaultProps} memoryUtilization={85} />);
    ...
    it('shows critical status for very high utilization', () => {
      const { container } = render(<HostAnalysisCard {...defaultProps} memoryUtilization={95} />);
